...
import {GetArticlesQuery} from 'storefrontapi.generated';

function Blog({blogs}: {blogs: Promise<GetArticlesQuery | null>}) {
  return (
    <>
      <Suspense fallback={<div>Loading blog...</div>}>
        <Await resolve={blogs}>
          {(blogs) => {
            const firstArticle = blogs?.result.listArticles[0];
            return (
              <div className="md:h-[700px] h-auto flex flex-col justify-around font-main w-screen mb-10">
                <div className="heading flex flex-row justify-between   w-[90%] mb-8 mx-auto items-center">
                  <div className="">
                    ...
                    <div className="title font-medium text-[40px]">
                      Latest Articles
                    </div>
                  </div>
                  <div className="blog-action underline text-[16px]">
                    View All
                  </div>
                </div>
