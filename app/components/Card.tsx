...
function Card({item}: {item: SimpleProduct}) {
  const [selectedOption, setSelectedOption] = useState('one');

  return (
    <div className="group card w-full md:w-[365px] h-[560px] bg-white rounded-lg font-main flex flex-col">
      ...
      <img
        ...
        alt=""
      />
      <div className="text visible w-[80%] flex flex-col flex-wrap self-center group-hover:hidden transition-all duration-200 ease-in-out ">
        <div className="tags mb-4  ">
          <span className="bg-[#F6F6F5] text-[14px] p-2.5 w-fit mr-4">
            • GMO Free
          </span>
          <span className="bg-[#F6F6F5] text-[14px] p-2.5 w-fit mr-4">
            • GMO Free
        ...
        <div className="subtitle text-gray-500">{item.description}</div>
        <div className="bottom-section flex flex-row justify-between mt-2">
          <div className="score flex flex-row items-center">
            {[...Array(5)].map((_, index) => (
              <BsStarFill key={index} />
            ))}
          </div>
          <div className="add-button p-1.5 w-28 bg-black rounded-lg text-white font-main text-xs text-center">
            Add • $49.95
          </div>
        </div>
      </div>
      ...
      <div className="hidden font-main group-hover:flex flex-col items-center w-100 group-hover:visible transition-all duration-200 ease-in-out ">
