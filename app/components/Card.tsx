...
function Card({
  ...
  const {open} = useAside();
   {/* <AddToCartButton variantId="gid://shopify/ProductVariant/51111751811386"> */}
  //const product = useProduct();
  //  const {product, variants, setSelectedVariant} = useProduct();
  console.log(item, 'productos desde card page');
  console.log(item.images.nodes[0].url)
  return (
...
>
 ...
 >
   ...
   <div className="bottom-section flex flex-row justify-between mt-2 gap-1.5 md:gap-0">
     ...
     >
        {price
         ? ` Add • $${item.options[0].optionValues[0].firstSelectableVariant?.price.amount}`
         : 'Add to Cart'} 
     </div>
   </div>
 </div>
