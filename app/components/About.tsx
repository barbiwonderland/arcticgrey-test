...
function About() {
  return (
    <div className="main-about flex flex-col w-screen main-font h-[514px] justify-center ">
      <div className="text w-[400px] h-32">
        <div className="heading text-[16px]">🧐 Why Health & Fitness</div>
        <div className="title text-[40px] font-medium">
          Clean Supplements - Made For You
        </div>
      </div>
      <div className="benefits mt-12 flex flex-row w-full gap-8">
        {features.map((item, index) => {
          return (
            <>
              ...
              >
                <div className="icon bg-black w-12 h-12 rounded-full mb-6 text-white flex items-center justify-center"><img src={item.icon} alt={item.title} /></div>
                <div className="title text-[18px] font-medium <BiLike /><GiThreeLeaves /><TbClipboardText />mb-4">
                  {item.title}
                </div>
                <div className="subtitle text-[16px] text-gray-700">{item.subtitle}</div>
              </div>
            </>
          );
        })}
